...
int main()
{
    ...
    for (int trailweight = BestWeight; trailweight < BestWeight + 1; trailweight++)
    {
        ...
        {
            int y[64];
            int P[64] = {48,1,18,35,32,49,2,19,16,33,50,3,0,17,34,51,52,5,22,39,36,53,6,23,20,37,54,7,4,21,38,55,56,9,26,43,40,57,10,27,24,41,58,11,8,25,42,59,60,13,30,47,44,61,14,31,28,45,62,15,12,29,46,63};
            for (int i = 0; i < 64; i++)
            {
                y[i] = xout[round][P[i]];
            }
        ...
        for (int round = 0; round < trailround; round++)
        {
            for (int i = 0; i < 16; i++)
            {
                AddProb[32 * round + i] = p[round][i];
                AddProb[32 * round + i + 16] = q[round][i];
            }
        }
